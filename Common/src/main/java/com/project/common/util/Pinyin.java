...
public class Pinyin {
    private static HashMap<String, String> PINYIN = new HashMap<String, String>();
	private static Pinyin pinyin = null;
    static {
        try {
            URL s = Pinyin.class.getClassLoader().getResource("pinyin.properties");
            PINYIN = pinyinToDic(new File(s.getPath()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
