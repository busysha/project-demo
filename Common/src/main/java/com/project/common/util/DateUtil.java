...
import com.project.common.dto.TimeWideDto;
...
public class DateUtil {
    ...
    /**
     * 返回时间戳范围 week 周 | day 当日 0点到次日0点
     * @param type
     * @return
     */
    public static TimeWideDto getTimeStampWide(String type){
        if("week".equals(type)){
            return getWeekTimeStamp();
        }else if("day".equals(type)){
            long now = System.currentTimeMillis();
            long today = dateTimeToLong(longToDate(now, "yyyy-MM-dd 00:00:00"));
            TimeWideDto timeWideDto = new TimeWideDto();
            timeWideDto.setBeginTime(today);
            timeWideDto.setEndTime(today + Constants.ONE_DAY_TIMESTAMP);
            return timeWideDto;
        }else if("month".equals(type)){
            long firstDay = dateToLong(getFirstDayToMonth());
            long lastDay = dateToLong(getLastDayToMonth()) + Constants.ONE_DAY_TIMESTAMP;
            TimeWideDto timeWideDto = new TimeWideDto();
            timeWideDto.setBeginTime(firstDay);
            timeWideDto.setEndTime(lastDay);
            return timeWideDto;
        }
        return null;
    }
     ...
     * 返回本周的时间戳范围
     */
    public static TimeWideDto getWeekTimeStamp(){

        Calendar calendar = Calendar.getInstance(TimeZone.getTimeZone("Asia/Shanghai"),Locale.SIMPLIFIED_CHINESE);
        calendar.setFirstDayOfWeek(Calendar.MONDAY);
        long now = dateToLong(longToDate(System.currentTimeMillis(),"yyyy-MM-dd"));
        ...
        long before = now - (dayOfWeek - 1)* Constants.ONE_DAY_TIMESTAMP;
        long after = now + ( 7 - dayOfWeek + 1) * Constants.ONE_DAY_TIMESTAMP;

        TimeWideDto timeWideDto = new TimeWideDto();
        timeWideDto.setBeginTime(before);
        timeWideDto.setEndTime(after);
