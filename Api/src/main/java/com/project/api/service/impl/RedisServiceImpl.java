...
public class RedisServiceImpl implements RedisService {
	...
	public Object deSerializeObject(String serializableStr , Class clazz){
		if(StringUtils.isEmpty(serializableStr)) return null;
		ObjectMapper mapper = ObjectMapperSingle.getInstance();
		mapper.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
		try {
			return mapper.readValue(serializableStr, clazz);
		} catch (Exception e) {
			logger.error(e);
		}
		return null;
	...
	@Override
	public double increaseNumByKeyAndInnerKey(final String key, final String value, final double num) {
		return template.execute(new RedisCallback<Double>() {
			@Override
			public Double doInRedis(RedisConnection connection) throws DataAccessException {
				return connection.zIncrBy(getBytesByString(key), num, getBytesByString(value));
			}
		});
	}
	...
	@Override
	public long hashSetIncr(final String key, final String innerKey, final long innerVal) {
		return template.execute(new RedisCallback<Long>() {
			@Override
			public Long doInRedis(RedisConnection redisConnection) throws DataAccessException {
